...
<div class="flex flex-col items-center justify-center w-full text-center md:w-4/5 md:text-center">
    <h1 class="mb-10 text-xl text-center text-indigo-500">Por favor primero completa tus datos</h1>
...
<div class="flex flex-col items-center justify-center w-full text-center md:w-4/5 md:text-center">
    <h4 class="mb-10 text-xl text-center text-indigo-500">No has registrado tus habilidades</h4>
    <a href="skills/show">
	...
	<div class="container">
		...
		<table class="flex flex-row flex-wrap w-full my-2 overflow-hidden rounded-lg sm:bg-white sm:shadow-lg">
                ...
                <thead class="text-white">
                    <tr class="flex flex-col flex-wrap mb-0.5 bg-indigo-400 rounded-l-lg sm:table-row sm:rounded-none sm:mb-0">
                        <th class="h-0.5 p-1 text-left">Skill</th>
                        <th class="p-3 text-left" width="110px">Acciones</th>
                    </tr>
                </thead>
                <tbody class="flex-1 sm:flex-none">
                    <tr class="flex flex-col mb-2 flex-no wrap sm:table-row sm:mb-0">
                        <td class="h-20 p-1 text-xs border border-grey-light lg:w-2/5 lg:text-xl sm:w-2/5 sm:text-xl hover:bg-gray-100">{{ $userSkill->skillName }}</td>
                        ...
                        <td class="p-3 text-red-400 border cursor-pointer lg:w-1/5 sm:w-1/5 border-grey-light hover:bg-gray-100 hover:text-red-600 hover:font-medium">
                            <form action="{{route('skills.destroy', $userSkill->skill_id)}}" method="POST" id={{$userSkill->skill_id}}>
                                @csrf
                                @method('DELETE')
                                <button type="button" class="bg-red-400 hover:bg-indigo-dark text-white font-bold py-2 px-4 rounded" onclick="deleteSkills({{$userSkill->skill_id}})">
                                    Eliminar
                                </button>
                            </a>
                        </form>
